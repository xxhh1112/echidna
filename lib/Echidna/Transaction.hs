...
import Echidna.Types.Buffer (forceBuf, forceLit, forceLitAddr)
...
genTx memo world txConf deployedContracts = do
  ...
  where
    ...
    toContractA sigMap (addr, c) =
      ...
      in (forceLitAddr addr,) <$> Map.lookup metadata sigMap
...
setupTx tx@Tx{call} = fromEVM $ do
  ...
  where
    incrementBalance = #env % #contracts % ix (LitAddr tx.dst) % #balance %= (\v -> Lit $ forceLit v + tx.value)
    encode (n, vs) = abiCalldata (encodeSig (n, abiValueType <$> vs)) $ V.fromList vs

--resetState :: FrameState s -> FrameState s
